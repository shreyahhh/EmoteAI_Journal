...
const CycleView = ({ user, entries, cyclePeriods, onPeriodsUpdated }) => {
  ...
  const chartMuted = isDark ? '#cbb28c' : '#7a5c3e';
  const chartGrid = isDark ? '#4d3620' : '#e0c9a0';
  const now = new Date();
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="emote-panel">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h3 className="text-emote-section font-semibold text-emote-ink">Calendar</h3>
            <p className="mt-1.5 text-emote-muted leading-relaxed text-emote-ink-soft">
              Mark when your period <span className="font-semibold text-emote-ink">started</span>, then when it{' '}
              <span className="font-semibold text-emote-ink">ended</span> (or say it&apos;s still ongoing). Past ranges
              show in soft color.
            </p>
          </div>
          <button type="button" onClick={startNewFromCalendar} className="emote-btn-primary shrink-0">
        ...
        <div className="mt-4 flex items-center justify-between gap-2 border-b border-emote-border pb-3">
          <button
            type="button"
            onClick={prevMonth}
            className="emote-btn-ghost flex h-9 min-w-9 items-center justify-center rounded-lg px-2 text-emote-body font-semibold"
        ...
        {calendarMode || editingId ? (
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <label className="flex cursor-pointer items-center gap-2 text-emote-muted text-emote-ink-soft">
              <input type="checkbox" checked={ongoing} onChange={(e) => setOngoing(e.target.checked)} className="h-4 w-4 rounded border-emote-border-strong text-[#a8432f] focus:ring-[#a8432f]/40" />
              Still ongoing (no end date yet)
            </label>
            <button type="button" onClick={resetForm} className="emote-btn-ghost py-2 text-emote-muted">
              Cancel
            </button>
          </div>
        ) : null}
        ...
        {showForm ? (
          <form onSubmit={handleSave} className="mt-6 space-y-5 border-t border-emote-border pt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {SYMPTOM_OPTIONS.map((s) => (
                  <button
                    ...
                    className={`rounded-full px-3.5 py-1.5 text-emote-muted font-medium transition focus:outline-none focus-visible:ring-2 focus-visible:ring-emote-gold/50 ${
                      symptoms.includes(s.id)
                        ? 'bg-gradient-to-r from-emote-accent-2/20 to-emote-gold/20 text-emote-accent-2 ring-1 ring-emote-gold/40'
            ...
            <div>
              <label htmlFor="cycle-mood" className="mb-1.5 block text-emote-caption font-medium uppercase tracking-wide text-emote-ink-faint">
                How do you feel?
              </label>
              <textarea
                id="cycle-mood"
                ...
                rows={3}
                className="emote-input min-h-[88px] resize-y"
                placeholder="Energy, mood, anything you want to remember about this period…"
              />
            </div>

            <div>
              <label htmlFor="cycle-notes" className="mb-1.5 block text-emote-caption font-medium uppercase tracking-wide text-emote-ink-faint">
                Other notes <span className="font-normal text-emote-ink-faint">(optional)</span>
              </label>
              <textarea
                id="cycle-notes"
                value={notes}
                ...
                rows={2}
                className="emote-input min-h-[72px] resize-y"
                placeholder="Anything else — sleep, meds, life context…"
              />
            </div>

            <div className="flex justify-end gap-2">
              <button type="button" onClick={resetForm} className="emote-btn-ghost">
                Discard
              </button>
              <button type="submit" disabled={saving} className="emote-btn-primary">
      ...
      <section className="emote-panel">
        <h3 className="text-emote-card-title font-semibold text-emote-ink">Journal tone: period days vs other days</h3>
        <p className="mt-1.5 text-emote-muted leading-relaxed text-emote-ink-soft">
        ...
        {cyclePeriods.length === 0 ? (
          <p className="mt-6 text-center text-emote-muted text-emote-ink-faint">Save at least one period to see this chart.</p>
        ) : (
      ...
      <section className="emote-panel">
        <h3 className="text-emote-card-title font-semibold text-emote-ink">Saved periods</h3>
        {sortedPeriods.length === 0 ? (
          <p className="mt-4 text-emote-muted text-emote-ink-faint">None yet — use &quot;New period on calendar&quot; above.</p>
        ) : (
          <ul className="mt-4 divide-y divide-emote-border">
            {sortedPeriods.map((p) => (
              <li key={p.id} className="py-4 first:pt-0">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex shrink-0 gap-2">
                    <button type="button" onClick={() => beginEdit(p)} className="emote-btn-ghost py-2 text-emote-muted">
                      Edit
                    </button>
                    <button type="button" onClick={() => handleDelete(p.id)} className="emote-btn-ghost py-2 text-rose-800 ring-rose-800/30">
                      Delete
                    </button>
                  </div>
                </div>
