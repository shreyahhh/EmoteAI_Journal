            ...
            <div className="bg-gray-800 rounded-2xl shadow-2xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                ...
                {isLoading ? (
                    ...
                    <div className="text-gray-300 space-y-6">
                        <div>
                            <h3 className="text-lg font-semibold text-purple-400 mb-2">Top Tips</h3>
                            <ul className="list-disc list-inside space-y-2">
                                {content.tips?.map((tip, index) => <li key={index}>{tip}</li>)}
                            </ul>
                        </div>
                        <div>
                            <h3 className="text-lg font-semibold text-purple-400 mb-2">A Simple Exercise: {content.exercise?.title}</h3>
                            <ol className="list-decimal list-inside space-y-2">
                                {content.exercise?.steps?.map((step, index) => <li key={index}>{step}</li>)}
                            </ol>
                        </div>
                         <div>
                            <h3 className="text-lg font-semibold text-purple-400 mb-2">Positive Affirmation</h3>
                            <p className="text-center italic text-xl border-t border-b border-purple-800 py-4">"{content.affirmation}"</p>
                        </div>
                    </div>
                )}
            </div>
